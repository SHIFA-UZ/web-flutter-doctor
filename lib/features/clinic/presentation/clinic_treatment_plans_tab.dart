...
import 'package:shifa_doc_app_v1/features/admin/presentation/admin_pdf_downloader_stub.dart'
    if (dart.library.html) 'package:shifa_doc_app_v1/features/admin/presentation/admin_pdf_downloader_web.dart' as clinic_pdf_dl;
...
class _ClinicTreatmentPlansTabState
                ...
                DataCell(
                  Row(
                    ...
                    children: [
                      IconButton(
                        tooltip:
                            l10n.translate('clinicTreatmentPlanExportPdf'),
                        icon:
                            const Icon(Icons.picture_as_pdf, size: 20),
                        onPressed: () =>
                            _exportTreatmentPlanPdf(context, p),
                      ),
                      IconButton(
  ...
  Future<void> _exportTreatmentPlanPdf(
    BuildContext context,
    TreatmentPlanSummaryDto plan,
  ) async {
    final l10n = AppLocalizations.of(context)!;
    ...
    try {
                ...
                children: [
                  ...
                  Expanded(
                    child: Text(l10n.translate(
                      'clinicTreatmentPlanExportPdfPreparing',
                    )),
                  ),
                ],
              ),
      ...
      try {
        detail = await fetchTreatmentPlanDetail(ref, plan.id);
      } catch (_) {
        if (!context.mounted) return;
      ...
      final fname = 'treatment_plan_${plan.id}_$ymd.pdf';
